...
const databsePath = path.join(__dirname, "todoApplication.db");
...
//API3 
app.post("/agenda/", async (request, response) => {
        ...
        if (status === "TO DO" || status === "IN PROGRESS" || status === "DONE"){
            if(category === "WORK" || category === "HOME" || category === "LEARNING"){
                if(isMatch(dueDate, "yyyy-MM-dd")){
                    ...
                    const postTodoQuery = `INSERT INTO todo (id, todo, priority, status, category, dueDate) 
                    VALUES
